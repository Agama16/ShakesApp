...
router.get('/history', async(req,res)=>
{
   ...
   //  const history=[]
     res.render("../View/customer/getHistory", {history})
...
router.get('/deleteAllHistory', async(req,res)=>
{
    const cust= req.session.userId
    await serviceCust.deleteAllHistory(cust)
    res.redirect("/menu/")
})

router.get('/deleteOneHistory/:id', async(req,res)=>{
    const id= req.params.id
    const cust= req.session.userId
    await serviceCust.deleteHistoryOne(cust,id)
    const history= await serviceCust.getCustHistoryById(cust)
   //  const history=[]
    //  res.render("../View/customer/getHistory", {history})
    res.redirect("/menu/history")
